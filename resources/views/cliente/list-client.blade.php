...
@section('title', 'Dashboard - Listado de Cliente')

@section('contenido-admin')
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-3xl font-semibold">Lista de Cliente</h2>
    ...
    <div class="lg:col-span-3 bg-white rounded-lg shadow-md p-6">
        ...
        <ul class="space-y-2">
            <li class="flex items-center">
                ...
                <span>Nuevo Cliente registrado: Admin. Juan Pérez</span>
            </li>
        </ul>
        <br><br>
    </div>
    ...
    <div class="overflow-x-auto p-4 bg-white rounded-lg">
        ...
        <br>
        <table id="client-table" class="min-w-full bg-black border border-gray-200 shadow-md rounded-lg">
            <thead class="bg-black text-white">
                <tr>
                    ...
                    <th class="px-6 py-3 border-b-2 border-gray-300 text-left text-sm font-semibold">Dirección</th>
...
@section('js')
    <script>
        $(document).ready(function() {
            var datatable = $('#client-table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: '{{ route('cliente.list_client') }}',
                    type: 'GET',
                },
                columns: [{
                        data: 'nombre',
                    ...
                    },
                    {
                        ...
                        render: function(data, type, row) {
                            let deleteUrl = `{{ url('administrador/cliente/${row.id}') }}`;
                            let editUrl = `{{ url('administrador/cliente/${row.id}/editar') }}`;
                            let viewUrl = `{{ url('administrador/cliente/${row.id}/ver') }}`;
                            return `
